...
import {logoutAction} from '../../js/actions/index'
...
class Navbar extends Component {
    constructor(props){
        ...
        //this.handleAccount  = this.handleAccount(this);
    ...
    handleLogout = async () => {
        ...
        await this.props.logoutAction(data);
    ...
    render()
    {
        ...
        if(cookie.load('cookie')){
            ...
            navLogin = (               
                <ul class="nav navbar-nav navbar-right">
                        <li><Link to="" onClick = {this.handleLogout}><span class="glyphicon glyphicon-user" style = {{color : "black"}}></span><label style = {{color: "black", fontWeight: "100"}}>{JSON.parse(sessionStorage.getItem("user")).UserName}</label></Link></li>                        
                        <li><Link to="/login" onClick = {this.handleLogout}><span class="glyphicon glyphicon-user" style = {{color : "black"}}></span><label style = {{color: "black", fontWeight: "100"}}>Logout</label></Link></li>                        
                </ul>                  
            );
            navLeft = (
               <div style = {{background: "#ace4d6", width: "15%", float: "left", height: "100vh", marginTop: "-2%"}}>
                    <ul style = {{listStyleType : "none", marginTop: "10%"}}>
                        <li><Link to="/home"><span class = "glyphicon" style = {{color : "black"}}></span><label style = {{color: "black", fontWeight: "100", cursor: "pointer"}}>Dashboard</label></Link></li>                                            
                        <li><Link to="/account"><span style = {{color : "black"}}></span><label style = {{color: "black", fontWeight: "100", cursor: "pointer"}}>Profile</label></Link></li>
                        <li><Link to="/createNewGroup"><span style = {{color : "black"}}></span><label style = {{color: "black", fontWeight: "100", cursor: "pointer"}}>Create New Group</label></Link></li>
                        <li><Link to="/groups"><span style = {{color : "black"}}></span><label style = {{color: "black", fontWeight: "100", cursor: "pointer"}}>Groups</label></Link></li>
                        <li><Link to="/myGroups"><span style = {{color : "black"}}></span><label style = {{color: "black", fontWeight: "100", cursor: "pointer"}}>My Groups</label></Link></li>
                        <li><Link to="/recentActivity"><span style = {{color : "black"}}></span><label style = {{color: "black", fontWeight: "100", cursor: "pointer"}}>Recent Activity</label></Link></li>
            ...
            navLogin = (
                <ul class="nav navbar-nav navbar-right">
                        <li><Link to="/login" style = {{color: "black"}}><span class="glyphicon glyphicon-log-in"></span> Login</Link></li>                        
                        <li><Link to="/Signup" style = {{color: "black"}}><span class="glyphicon glyphicon-log-in"></span> Signup</Link></li>
                </ul>
            )
        }
        ...
        return(
            <div>
                {redirectVar}
            <nav class="navbar navbar-inverse" style = {{backgroundColor: "rgb(172, 228, 214)", borderColor: "white"}}>
                <div class="container-fluid">
                    <div class="navbar-header">
                        {/* <a class="navbar-brand">Book Store App</a> */}
                        {/* <a href="#" class="text-teal nav-icon" data-item="plane"> */}
...
const Nav = connect(mapStateToProps, {logoutAction})(Navbar);
export default Nav;
